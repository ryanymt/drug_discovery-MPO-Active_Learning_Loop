...
To optimize molecules, they must be measured. A multi-tiered scoring strategy is used.
...
Instead of scoring sequentially, massive parallel jobs are launched on **Google Cloud Batch**.
*   **Batch Size:** 1,000 - 100,000 molecules.
*   **Scripting:** Unified Bash scripts (`run_gnina_loop2.sh`) are used to iterate through files on GCS mounts, avoiding data transfer bottlenecks.
