...
## 6. Reference Case Study: The Smoothened Receptor (4YHJ)

To validate this architecture, we deployed a reference implementation targeting a classic "hard problem" in Small Molecule Drug Discovery: Multi-Parameter Optimization (MPO). The objective was to identify drug candidates that simultaneously balance high potency, safety (low toxicity), and synthesizability for the **Human Smoothened Receptor (PDB: 4YHJ)**.
...
1. **Generation (The Source):** We utilized a Generative AI model (based on the Pocket2Mol architecture) to "hallucinate" a library of **100,000 candidates** targeting the 4YHJ pocket.
2. **Ground Truth (The Teacher):** It is too expensive to run physics simulations on all 100,000 molecules. Instead, the Strategist selects a small subset (approx. 1,000 molecules) to undergo rigorous Free Energy Perturbation (FEP) simulations.
   * **Strategic Sampling (90/10 Rule):** To ensure the downstream models learn the difference between "good" and "bad," the Strategist employs an Exploration-Exploitation strategy. It selects 90% of the candidates predicted to be high potency, but reserves 10% to probe high uncertainty or "unfit" regions of the chemical space. This ensures the Proxy model learns the true decision boundary rather than just memorizing the winners.
...
### 6.2 The Results:  Hit Rate Improvement
...
* **The Outcome:** The entire simulation batch completed in under **3 hours**.
* **Impact:** This represents a >95% reduction in Time-to-Insight.

Comparing the "Baseline" (Cycle 1) to the "Active Learning" (Cycle 2) populations, the system achieved a **Hit Rate of 94.45%** (a +2.32% improvement over the baseline) and shifted the mean affinity to **-36.89 kcal/mol**. This confirms that the loop is capable of refining an already high-performing population, pushing the boundaries of the chemical space further.
