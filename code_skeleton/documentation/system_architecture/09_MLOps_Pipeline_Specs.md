...
## 1. Overview
The platform uses **Vertex AI Pipelines (Kubeflow)** to orchestrate the multi-stage drug discovery workflow. We maintain two distinct versions of the pipeline to balance development speed with production accuracy.
...
While the pipeline runs, we are using manual paths in GCS. 
