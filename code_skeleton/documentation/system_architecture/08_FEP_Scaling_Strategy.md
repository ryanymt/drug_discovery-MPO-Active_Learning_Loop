...
*   **Scaling Behavior**:
    ...
    *   **Spot Risk**: **Severe**. With 13,000 tasks, Preemption Rate is a probability game. If the rate is 5% per hour, probability of a 7-hour task surviving is $(0.95)^7 \approx 70\%$. You will retry ~30% of tasks.
    *   **Cost Impact**: You pay for the 30% waste.
*   **Verdict**: Viable only if you limit concurrency (e.g., "Rolling Batch" of 50 molecules at a time) to avoid triggering aggressive preemption logic in the zone.
...
*   **Scaling Behavior**:
    ...
    *   **Throughput**: Higher effective throughput because you don't lose progress.
...
3.  **Result**:
    ...
    *   **Benefit**: Keeps us within Quota limits and smooths out the "Startup Storm" over time without writing any custom queue logic.

### The "Hybrid" Checkpoint
For the 1,000 molecule scale, **Option 2 (Checkpointing) is superior**.
*   **Reason**: At 13,000 tasks, you *will* hit maintenance windows, stockouts, and preemptions. Relying on luck (Option 1) for 13,000 dice rolls is statistically poor.
