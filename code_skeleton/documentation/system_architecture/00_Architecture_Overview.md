...
```mermaid
graph TD
    subgraph "Generative Layer (The Actor)"
        P2M[Pocket2Mol Generator]
        Retrain[Fine-Tuning Job]
    end

    subgraph "Filter Layer"
        Gnina[Gnina Docking]
        RDKit[RDKit Props]
        TxGemma[TxGemma ADMET]
    end

    subgraph "Oracle Layer (The Teacher)"
        FEP[GROMACS FEP (Batch)]
        Mock[Mock Oracle (Python)]
    end

    subgraph "Learning Layer (The Critic)"
        XGB[XGBoost Proxy]
    ...
    BQ -->|Metrics| Selector
    Selector -->|Candidates| FEP
    FEP -->|Delta G| BQ
    
    BQ -->|Training Data| XGB
    XGB -->|Predictions| Retrain
    Retrain -->|New Weights| P2M
```
