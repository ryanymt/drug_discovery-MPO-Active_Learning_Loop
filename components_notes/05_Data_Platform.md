...
*   `[INPUT_BUCKET]`: Immutable references (Protein PDBs, Configs, Scripts).
*   `[OUTPUT_BUCKET]`: Pipeline outputs.
    ...
    *   `/oracle_batches`: Aggregated SDFs for validation simulations.

## 2. Warehouse: BigQuery (The House)
Used for structured metrics, querying, and analytics.

**Dataset:** `[PROJECT_ID].bioops_platform`
...
*   **Key**: `molecule_hash`.
*   **Content**: `final_deltag` (Binding Free Energy).
*   **Logic**: Populated only after expensive validation runs.

#### 4. `elite_candidates_v1` (Active Learning)
*   **Role**: The "Platinum" Layer.
*   **Key**: `molecule_hash`.
*   **Content**: Top candidates selected for Retraining Cycle.

## 3. Data Movement Strategy

### The "Upsert" Pattern
To handle the massive redundancy in generative models (where the same molecule is generated thousands of times), BigQuery is treated as an Idempotent Store:
...
SQL logic is used to present a single `final_score` to downstream consumers:
```sql
